...
for time in times:
    ...
    for k in xrange(0, (numRuns/numParallelJobs)):
        ...
        if "pomcp" in variant:
            configFilePath = configFolder + robot + "/cfg/" + robot + "_pomcp" + "_$SLURM_ARRAY_TASK_ID.cfg"
            string += "./despotSolver --cfg " + configFilePath + " \n"
        else:
            configFilePath = configFolder + robot + "/cfg/" + robot + "_" + variant + "_$SLURM_ARRAY_TASK_ID.cfg"
        ...
        if (os.path.exists(folder + "/jobs_abt_" + variant + "_" + robot + "_" + str(k) + ".sh")):
            os.remove(folder + "/jobs_abt_" + variant + "_" + robot + "_" + str(k) + ".sh")
        with open(folder + "/jobs_abt_" + variant + "_" + robot + "_" + str(k) + ".sh", 'a+') as f:
            f.write(string)
    print "HELLO"
