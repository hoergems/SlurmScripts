...
for i in xrange(1, numConvarianceSteps + 1):
	...
	for k in xrange(0, numRuns/numParallelJobs):	    
	    ...
	    string += "./abt_robot --cfg /data/hoe01h/ConfigFiles/WAFRJournal/largeErrors/DubinMaze/cfg/" + folder2	    
	    string += "/DubinMaze_$SLURM_ARRAY_TASK_ID.cfg \n"        
...
for i in xrange(1, numConvarianceSteps + 1):
	...
	for k in xrange(0, numRuns/numParallelJobs):	    
	    ...
	    string += "export GAZEBO_MASTER_URI=http://localhost:$gzMasterUriPort \n"
	    string += "cd /data/hoe01h/tapir_devel/bin \n"
	    string += "./mhfr_robot --cfg /data/hoe01h/ConfigFiles/WAFRJournal/largeErrors/DubinMaze/cfg/" + folder2	    
	    string += "/DubinMaze_$SLURM_ARRAY_TASK_ID.cfg \n"        
	    if not os.path.exists(folder + "/" + str(i) + "_proc_" + str(j) + "_obs"):
		os.makedirs(folder + "/" + str(i) + "_proc_" + str(j) + "_obs")
