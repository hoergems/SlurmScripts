...
parser.add_argument('-a', '--alg', type=str, required=True, help="Algorithm")
parser.add_argument('-t', '--time', type=int, required=True, help="time")
...
for i in xrange(1, numConvarianceSteps + 1):
	...
	for k in xrange(0, numRuns/numParallelJobs):	    
	    ...
	    string += "#SBATCH --ntasks=1 \n"
            string += "#SBATCH --cpus-per-task=" + str(cpus_per_task) + " \n"
	    ...
	    string += "cd /data/hoe01h/oppt_devel/bin \n"
            string += "./" + alg + " --cfg " + configFolder + robot + "/cfg/" + folder2	    
	    ...
	    if (os.path.exists(folder + "/" + str(i) + "_proc_" + str(j) + "_obs" + "/jobs_" + alg + "_" + robot + "_" + str(k) + ".sh")):
		os.remove(folder + "/" + str(i) + "_proc_" + str(j) + "_obs" + "/jobs_" + alg + "_" + robot + "_" + str(k) + ".sh")
	    with open(folder + "/" + str(i) + "_proc_" + str(j) + "_obs" + "/jobs_" + alg + "_" + robot + "_" + str(k) + ".sh", 'a+') as f:
		f.write(string)
